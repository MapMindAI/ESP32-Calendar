...
| `officer_label` | 16, 208 | `建除：<十二建除>`, 12 px Chinese subset | `Calendar_LoopTask` | on date change |
| `deity_label` | 16, 226 | `值神：<十二值神>`, 12 px Chinese subset | `Calendar_LoopTask` | on date change |
...
| `lv_font_calendar_yiji_12` | 12 px | 1 | the `宜：` / `忌：` lines in the bottom bar (one clipped line each) and the `建除：` / `值神：` lines in the left column |
...
list plus the fixed `宜忌无：十二建除值神` wording and all 十二建除 / 十二值神 names (223 glyphs, ~4.7 KB),
and `lv_font_calendar_chinese_12` holds 今、吉、日、道、非、黄 for the 黄道吉日 wording. Add new Chinese
dashboard wording by regenerating an explicit 1 bpp subset first.
