...
static lv_obj_t* make_container(lv_obj_t* parent, lv_coord_t x, lv_coord_t y, lv_coord_t w,
                                lv_coord_t h) {
...
static lv_obj_t* make_label(lv_obj_t* parent, lv_coord_t x, lv_coord_t y, lv_coord_t w,
                            const lv_font_t* font, lv_text_align_t align, lv_coord_t letter_space,
                            const char* text) {
...
void calendar_ui_create(lv_obj_t* parent) {
  ...
  weekday_label =
      make_label(left_panel, 2, 28, 132, &lv_font_calendar_12, LV_TEXT_ALIGN_LEFT, 1, "");
  ...
  month_label =
      make_label(right_panel, 0, 0, RIGHT_W, &lv_font_calendar_18, LV_TEXT_ALIGN_LEFT, 0, "");
  ganzhi_label = make_label(right_panel, RIGHT_W - 48, 3, 48, &lv_font_calendar_ganzhi_12,
                            LV_TEXT_ALIGN_RIGHT, 0, "");

  for (int col = 0; col < CAL_COLS; col++) {
    make_label(right_panel, CAL_GRID_X + col * CAL_CELL_W, CAL_HEADER_Y, CAL_CELL_W,
               &lv_font_calendar_16, LV_TEXT_ALIGN_CENTER, 0, weekday_initials[col]);
  }

  for (int row = 0; row < CAL_ROWS; row++) {
    for (int col = 0; col < CAL_COLS; col++) {
      lv_obj_t* cell =
          make_label(right_panel, CAL_GRID_X + col * CAL_CELL_W, CAL_GRID_Y + row * CAL_CELL_H,
...
lv_obj_t* calendar_ui_root(void) { return root; }
...
void calendar_ui_refresh_all(const calendar_ui_data_t* data) {
  ...
  calendar_ui_update_environment(data->temperature_c, data->humidity_percent,
                                 data->environment_valid);
  ...
  if (data->lunar_data_valid) {
    ...
    static const char* const zhi[] = {"子", "丑", "寅", "卯", "辰", "巳",
                                      "午", "未", "申", "酉", "戌", "亥"};
    static const char* const officer_names[] = {"建", "除", "满", "平", "定", "执",
                                                "破", "危", "成", "收", "开", "闭"};
    static const char* const deity_names[] = {"青龙", "明堂", "天刑", "朱雀", "金匮", "天德",
                                              "白虎", "玉堂", "天牢", "玄武", "司命", "勾陈"};
    snprintf(buf, sizeof(buf), "%s%s年", gan[data->lunar_year_gan], zhi[data->lunar_year_zhi]);
    lv_label_set_text(ganzhi_label, buf);
    snprintf(buf, sizeof(buf), "建除：%s 值神：%s", officer_names[data->lunar_officer_index],
             deity_names[data->lunar_deity_index]);
